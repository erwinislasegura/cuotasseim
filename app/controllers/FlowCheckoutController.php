...
class FlowCheckoutController extends Controller
{
    ...
    private function findSocioDataByRut(string $rutInput): ?array
    {
        ...
        $this->ensurePendingCuotasFromPlan((int) ($socio['id'] ?? 0), 2);
    ...
    private function ensurePendingCuotasFromPlan(int $socioId, int $targetPending = 2): void
    {
        if ($socioId <= 0 || $targetPending <= 0) {
            return;
        }
        ...
        while ($attempts < 5) {
            ...
            $stmtCount->execute();
            $pendingCount = (int) ($stmtCount->fetchColumn() ?: 0);

            if ($pendingCount >= $targetPending) {
                return;
            }

            $createdId = $this->createCuotaFromCurrentPlan($db, $socioId);
            if ($createdId <= 0) {
                return;
            }
        }
    ...
    private function createCuotaFromCurrentPlan(\PDO $db, int $socioId): int
    {
        $stmt = $db->prepare("SELECT sp.periodo_id, p.tipo_periodo, p.monto_a_pagar
        ...
        $tipoPeriodo = (string) ($plan['tipo_periodo'] ?? 'mensual');
        $fechaVencimiento = $this->nextDueDateByPlan($db, $socioId, $periodoId, $tipoPeriodo);
        ...
        $stmtExiste->bindValue(':periodo_id', $periodoId, \PDO::PARAM_INT);
        $stmtExiste->bindValue(':fecha_vencimiento', $fechaVencimiento);
        $stmtExiste->execute();
        $existing = $stmtExiste->fetch();
        if ($existing) {
        ...
        $stmtInsert->bindValue(':periodo_id', $periodoId, \PDO::PARAM_INT);
        ...
        $stmtInsert->bindValue(':observacion', 'Generada automáticamente desde Pago Flow');
        $stmtInsert->execute();

        return (int) $db->lastInsertId();
    }

    private function nextDueDateByPlan(\PDO $db, int $socioId, int $periodoId, string $tipoPeriodo): string
    {
        if ($socioId <= 0 || $periodoId <= 0) {
            return $this->currentPeriodDueDate($tipoPeriodo);
        }
        ...
        $startPeriod = $base->modify('+' . ($cuotasRegistradas * $jumpMonths) . ' months');
        $endPeriod = $startPeriod->modify('+' . ($jumpMonths - 1) . ' months')->modify('last day of this month');

        return $endPeriod->format('Y-m-d');
    }

    private function currentPeriodDueDate(string $tipoPeriodo): string
    {
        $year = (int) date('Y');
        $month = (int) date('n');
